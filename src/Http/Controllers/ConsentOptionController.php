...
class ConsentOptionController extends Controller
{
	public function index(ConsentOptionsDatatables $dataTable)
	{
		...
		return view('vendor.ekoukltd.laraconsent.consent-options.index')
			->with([
	...
	public function edit(ConsentOption $consentOption)
	{
		...
		if(!$consentOption->isHighestVersion) {
			...
			return redirect(
				route(
					config('laraconsent.routes.admin.prefix').'.edit', $editableVersion
				)
			)->with(['success' => 'Version '.
			                      $consentOption->version.
			                      ' is not editable. Editing current version '.
